...
if not STOCKS:
    ...
    if len(STOCKS) == 1:
        st.warning(
            "You've selected only one stock. Some visualizations might not be as informative without a "
            "comparison."
        )
    with col2:
        ...
        pie_chart = (
            alt.Chart(df)
            .mark_arc(innerRadius=50, outerRadius=150)
            .encode(
                theta="Market Cap:Q", color="Ticker:N", tooltip=["Ticker", "Market Cap"]
            )
        )
    ...
    with col3:
        ...
        fifty_day_avg_df = pd.DataFrame(
            list(fifty_day_avg_values.items()), columns=["Ticker", "50 Day Avg"]
        )
        st.write("50 Day Moving Averages")
        horizontal_bar_chart = (
            alt.Chart(fifty_day_avg_df)
            .mark_bar()
            .encode(
    ...
    if len(STOCKS) == 1:
        st.warning(
            "Modern Portfolio Theory could not be processed as only one stock is selected"
        )
    else:
        with st.spinner("Loading..."):
            ...
            with col1:
                ...
                optimal_weightings_df = pd.DataFrame(
                    [weights_list[min_vol_idx]], columns=STOCKS
                )
            ...
            with col2:
                ...
                chart = (
                    alt.Chart(portfolios)
                    .mark_circle()
                    .encode(x="Volatility", y="Return")
                    .interactive()
                )
        ...
        for _ in range(days):
            with torch.no_grad():
                last_sequence_tensor = torch.tensor(
                    last_sequence.reshape(1, seq_length, -1), dtype=torch.float32
                ).to(device)
    ...
    for stock in bars:
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 10 == 0:
                print(
                    f"Stock: {stock}, Epoch [{epoch}/{num_epochs}], Loss: {loss.item()}"
                )
        ...
        combined_data = pd.concat(
            [stock_data[-30:], predictions_series.rename("Prediction")], axis=1
        )
